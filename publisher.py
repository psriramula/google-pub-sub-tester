...
def publish_messages(project_id, topic_name):
    ...
    #    Override this to send the test data for a given test case
    #    Also, to keep the calling mechanisum in sync , read the test case type if possible as a parameter to this
...
def publish_data_messages(project_id, topic_name, data):
    """Publishes multiple messages to a Pub/Sub topic."""
    from google.cloud import pubsub_v1
    publisher = pubsub_v1.PublisherClient()
    topic_path = publisher.topic_path(project_id, topic_name)
...
def simulate_delay():
    rand_milli_sec = random.randint(1, 60)
    delay = 10 / rand_milli_sec
    sleep(delay)


def publish_messages_from_test_suite(project_id, topic_name, test_suite_number):
    """Publishes multiple messages to a Pub/Sub topic."""
    # [START pubsub_quickstart_publisher]
    # [START pubsub_publish]
    from google.cloud import pubsub_v1
    ...
    path = "./testdata/" + test_suite_number + '.txt'
    file = open(path)
    line = file.readline()
    while line:
        yield line
